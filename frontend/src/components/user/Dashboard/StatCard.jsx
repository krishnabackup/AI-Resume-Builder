...
const StatCard = React.memo(({ label, value, icon, trend, trendIcon, trendColor, isProbability, className = "" }) => {
  return (
    <div className={`bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between min-h-[105px] ${className}`}>
      ...
      <div className="flex flex-col mt-auto">
        ...
        {trend && (
          <span className={`${trendColor || "text-emerald-600"} text-[10px] flex items-center gap-1 font-medium mt-1`}>
            {trendIcon && React.cloneElement(trendIcon, { className: "text-[10px]" })}
            {trend}
          </span>
        )}
      </div>
